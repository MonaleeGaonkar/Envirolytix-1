...
const DinoGame: React.FC = () => {
  ...
  return (
    <div
      ...
      className="relative w-full max-w-3xl mx-auto h-52 mb-8 overflow-hidden rounded-lg shadow-inner bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 select-none cursor-pointer focus:outline-none"
      tabIndex={0}
    >
      <svg width="100%" height="100%" viewBox={`0 0 ${GAME_WIDTH} ${GAME_HEIGHT}`}>
        ...
        <text x={GAME_WIDTH - 10} y="20" textAnchor="end" className="font-mono text-lg font-bold fill-current text-gray-500">
        ...
        {(state.status === 'idle' || state.status === 'gameOver') && (
            <g>
                <rect x="0" y="0" width={GAME_WIDTH} height={GAME_HEIGHT} className="fill-current text-gray-100/70 dark:text-gray-800/70" />
                <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 - 20} textAnchor="middle" className="text-4xl font-bold fill-current text-gray-800 dark:text-gray-200">
                    {state.status === 'gameOver' ? 'Game Over' : 'Eco Jumper'}
                </text>
                 <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 + 20} textAnchor="middle" className="text-lg fill-current text-gray-500 dark:text-gray-400">
                    {state.status === 'gameOver' ? `Score: ${Math.floor(state.score)}` : `High Score: ${state.highScore}`}
                </text>
                <text x={GAME_WIDTH / 2} y={GAME_HEIGHT / 2 + 50} textAnchor="middle" className="text-md fill-current text-gray-500 dark:text-gray-400 animate-pulse">
